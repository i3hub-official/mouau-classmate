// File: lib/services/teacher/studentManagementService.ts

import { prisma } from "@/lib/server/prisma";
import { StudentBasicInfo, EnrolledStudent } from "@/lib/types/teacher/index";
...
export class TeacherStudentManagementService {
   ...
   */
  static async getCourseStudents(teacherId: string, courseId: string, page: number = 1, limit: number = 10) {
    try {
   ...
   */
  static async getStudentPerformanceSummary(teacherId: string, studentId: string) {
    try {
      // Get enrollments for courses taught by this teacher
      const enrollments = await prisma.enrollment.findMany({
        where: {
      ...
      return {
        ...
        gradingRate: submittedAssignments > 0 ? (gradedAssignments / submittedAssignments) * 100 : 0,
      };
    } catch (error) {
