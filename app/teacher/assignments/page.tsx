...
export default function TeacherAssignmentsPage() {
  ...
  const filteredAssignments = assignments.filter((assignment) => {
    ...
    const matchesCourse =
      !filters.courseId || assignment.course.id === filters.courseId;
    ...
    const matchesStatus =
      ...
      (filters.status === "overdue" &&
        ...
        assignment.submissions.some((s) => !s.isGraded)) ||
      (filters.status === "graded" &&
        assignment.submissions.every((s) => s.isGraded));
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex gap-2">
          ...
          <select
            ...
            onChange={(e) =>
              setFilters((prev) => ({ ...prev, isPublished: e.target.value }))
            }
            className="px-3 py-2 border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary bg-background"
          >
...
function AssignmentCard({ assignment }: { assignment: Assignment }) {
  ...
  const isOverdue =
    new Date(assignment.dueDate) < new Date() && pendingGrading > 0;
        ...
        <div className="mb-4">
          ...
          <div className="w-full bg-muted rounded-full h-2">
            <div
              ...
              style={{
                width: `${(gradedSubmissions / totalSubmissions) * 100}%`,
              }}
            ></div>
          </div>
        </div>
