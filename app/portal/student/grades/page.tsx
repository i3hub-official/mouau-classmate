// app/portal/student/grades/page.tsx
      ...
      <div className="container mx-auto py-6">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-blue-600">{gradeStats.gpa.toFixed(2)}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">{gradeStats.academicStanding}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">{gradeStats.completedCourses}/{gradeStats.totalCourses}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-amber-600">{gradeStats.currentSemester}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 lg:col-span-2">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-indigo-600">{gradeStats.earnedCredits}/{gradeStats.totalCredits}</p>
              </div>
        ...
        <div className="space-y-6">
          {gradesData.courses.length === 0 ? (
            ...
            gradesData.courses.map((course) => (
              <div key={course.courseId} className="bg-white rounded-xl shadow-lg overflow-hidden">
                {/* Course Header */}
                <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="text-xl font-bold text-white">{course.courseCode}</h3>
                    ...
                    <div className="text-right">
                      <div className="inline-flex items-center bg-white/20 rounded-full px-3 py-1 text-white text-sm font-medium">
                        {course.averageScore.toFixed(1)}/{course.averageGradePoint.toFixed(1)}
                      </div>
                ...
                <div className="grid grid-cols-3 gap-4 p-6 bg-gray-50">
                  <div className="text-center">
                    <p className="text-2xl font-bold text-blue-600">{course.assignments.length}</p>
                  ...
                  <div className="text-center">
                    <p className="text-2xl font-bold text-amber-600">{course.averageGradePoint.toFixed(1)}</p>
                    <p className="text-sm text-muted-foreground">Avg. Grade Points</p>
                  </div>
                </div>
                ...
                <div className="p-6">
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span>{course.totalWeight > 0 ? Math.round((course.earnedWeight / course.totalWeight) * 100) : 0}%</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className="h-2 bg-blue-600 rounded-full"
                        style={{ width: `${course.totalWeight > 0 ? Math.round((course.earnedWeight / course.totalWeight) * 100) : 0}%` }}
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="p-6">
                  <h4 className="text-lg font-semibold mb-4">Assignment Grades</h4>
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b">
                          ...
                          <th className="text-left p-2 font-medium">Grade Points</th>
                            ...
                            <tr key={assignment.id} className="border-b hover:bg-gray-50">
                              <td className="p-3">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {assignment.assignment.weight > 0 && `Weight: ${assignment.assignment.weight}%`}
                                </div>
                              </td>
                              <td className="p-3">
                                <div className={cn(
                                  "text-sm",
                                  isPast(assignment.assignment.dueDate) && "text-red-500"
                                )}>
                                  {format(assignment.assignment.dueDate, "MMM dd, yyyy")}
                                </div>
                              </td>
                              <td className="p-3">
                              ...
                              <td className="p-3">
                                <span className={cn(
                                  ...
                                  assignment.grade === "A" && "bg-green-100 text-green-800",
                                  assignment.grade === "B" && "bg-blue-100 text-blue-800",
                                  assignment.grade === "C" && "bg-yellow-100 text-yellow-800",
                                  assignment.grade === "D" && "bg-orange-100 text-orange-800",
                                  assignment.grade === "F" && "bg-red-100 text-red-800"
                                )}>
                                  {assignment.grade}
                                </span>
                              </td>
                              <td className="p-3">
                                <span className="font-medium">{assignment.gradePoint}</span>
                              </td>
                              <td className="p-3">
                                <button
                                  onClick={() => {
                                    window.location.href = `/portal/student/assignments/${assignment.assignmentId}`;
                                  }}
  ...
  const courses: CourseGrades[] = enrollments.map((enrollment) => {
    ...
    gradedAssignments.forEach((assignment) => {
      ...
      if (submission) {
        ...
        earnedWeight += (submission.score || 0) >= (assignment.maxScore * 0.6) ? (assignment.weight || 0) : 0;
        ...
        gradesWithDetails.push({
          ...
          grade: assignment.maxScore && submission.score !== null
            ? getGradeFromScore(submission.score, assignment.maxScore)
            : "F",
          gradePoint: assignment.maxScore && submission.score !== null
            ? getGradePoints(getGradeFromScore(submission.score, assignment.maxScore))
            : 0,
    ...
    return {
      ...
      averageScore: gradedAssignments.length > 0 ? totalScore / gradedAssignments.length : 0,
      averageGradePoint: gradedAssignments.length > 0 ? totalGradePoints / gradedAssignments.length : 0,
      totalWeight,
      earnedWeight,
