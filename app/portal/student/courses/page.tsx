          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-purple-500">
                {allCourses.length > 0
                  ? Math.round(
                      allCourses.reduce((sum, c) => sum + c.enrollment.progress, 0) /
                        allCourses.length
                    )
                  : 0}
                %
        ...
        <div className="flex items-center justify-between px-6 py-3 border-t bg-card">
          ...
          <div className="flex items-center space-x-2">
            <button
              onClick={() => {
                ...
                if (page > 1) {
                  ...
                  window.location.href = `/portal/student/courses?${params.toString()}`;
                }
              }}
            ...
            <button
              ...
              disabled={parseInt(searchParams.page || "1", 10) >= Math.ceil(filteredCourses.length / 9)}
              className="inline-flex items-center justify-center rounded-md border border-input bg-background hover:bg-accent hover:text-accent-foreground h-8 px-3 py-2 disabled:opacity-50"
            >
              Next
            </button>
...
function CourseCard({ course }: { course: CourseWithDetails }) {
  const isInProgress = !course.enrollment.isCompleted && course.enrollment.progress > 0;
    ...
    <div className="group rounded-lg border bg-card overflow-hidden hover:shadow-lg transition-all duration-200 hover:-translate-y-1">
      ...
      <div className="p-6">
        ...
        <div className="flex space-x-2">
          <button
            onClick={() => {
              window.location.href = `/portal/student/courses/${course.id}`;
            }}
          ...
          <button
            onClick={() => {
              window.location.href = `/portal/student/courses/${course.id}/progress`;
            }}
...
function filterCourses(
  ...
  searchParams: { search?: string; level?: string; semester?: string; status?: string }
): CourseWithDetails[] {
  ...
  if (searchParams.level) {
    filtered = filtered.filter((c) => c.level.toString() === searchParams.level);
  }

  // Filter by semester
  if (searchParams.semester) {
    filtered = filtered.filter((c) => c.semester.toString() === searchParams.semester);
  }
  ...
  if (searchParams.status) {
    filtered = filtered.filter((course) => {
      switch (searchParams.status) {
        case "in-progress":
          return !course.enrollment.isCompleted && course.enrollment.progress > 0;
        case "completed":
