...
export default async function StudentDashboardPage() {
  ...
  const upcomingAssignments = await getUpcomingAssignments(session.user?.id || "");
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-slate-900">{stats.inProgressCourses}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-slate-900">{stats.pendingAssignments}</p>
              </div>
              ...
              <div className="space-y-4">
                {courses.length === 0 ? (
                  ...
                  courses.slice(0, 3).map((course) => (
                    <div key={course.id} className="border border-slate-200 rounded-lg p-4 hover:bg-slate-50 transition-colors">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          <h3 className="font-semibold text-slate-900">{course.title}</h3>
                          <p className="text-sm text-slate-600">{course.code} • Level {course.level}</p>
                        </div>
                      ...
                      {course.lastAccessedAt && (
                        <div className="flex items-center text-xs text-slate-500 mt-2">
                          ...
                          Last accessed {format(course.lastAccessedAt, "MMM d, yyyy")}
                        </div>
                      )}
                      
                      <Link
                        href={`/portal/student/courses/${course.id}`}
                    ...
                    <div key={assignment.id} className="border border-slate-200 rounded-lg p-4 hover:bg-slate-50 transition-colors">
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-slate-600">{assignment.courseCode} • {assignment.courseTitle}</p>
                        </div>
                        <div className="text-right">
                          <span className={cn(
                            "inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold",
                            isPast(assignment.dueDate) ? "bg-red-100 text-red-800" : "bg-amber-100 text-amber-800"
                          )}>
                            {isPast(assignment.dueDate) ? "Overdue" : format(assignment.dueDate, "MMM d")}
                          </span>
                        </div>
                      </div>
              ...
              <div className="space-y-3">
                {recentGrades.length === 0 ? (
                  ...
                  recentGrades.slice(0, 3).map((grade) => (
                    <div key={grade.id} className="border border-slate-200 rounded-lg p-4 hover:bg-slate-50 transition-colors">
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex-1">
                          <h3 className="font-semibold text-slate-900 truncate">{grade.assignmentTitle}</h3>
                          <p className="text-sm text-slate-600">{grade.courseCode} • {grade.courseTitle}</p>
                        </div>
                        <div className="text-right">
                      ...
                      <Link
                        href={`/portal/student/grades/${grade.id}`}
                        className="flex items-center text-blue-600 hover:text-blue-800 text-sm font-medium mt-3"
                      >
                        View Details
                        <ChevronRight className="h-4 w-4 ml-1" />
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200">
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <span className="font-medium text-slate-900">View Assignments</span>
                </Link>
                ...
                >
                  ...
                  <span className="font-medium text-slate-900">Academic Calendar</span>
                </Link>
                ...
                >
                  ...
                  <span className="font-medium text-slate-900">Learning Resources</span>
                </Link>
              </div>
            </div>
          </div>
...
async function getUpcomingAssignments(userId: string): Promise<AssignmentWithDeadline[]> {
